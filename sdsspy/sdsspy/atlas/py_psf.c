...
PyObject* py_read_kl_basis(const char* filename, int ext) {
    ...
    basis = py_load_kl_basis(filename, ext);
    if (basis == NULL) {
        return NULL;
    }
...
/* had to move fits deletion into this program in order to get
 * around gcc 4.3 bug:  for some reason if I returned the FITS
 * structure it would not free up the file descriptors after
 * I ran close()  ESS 2011-08-24
 */
PSF_BASIS* py_load_kl_basis(const char* filename, int ext) {
    FITS *fits;
    PSF_BASIS *basis = NULL;		/* PSF_BASIS derived from kl_comps */
    ...
    basis = py_copy_kl_basis(fits);
    phFitsDel(fits);  

    return basis;
}

PSF_BASIS* py_copy_kl_basis(FITS* fits) {
